...
namespace AssaultBird2454.VPTU.SaveEditor.UI.Pokedex
{
    ...
    public partial class Pokemon : Window
    {
        ...
        public Pokemon(SaveManager.SaveManager _Mgr, PokemonData _PokemonData = null)
        {
        ...
        {
            var Select = new Search_Resources(Mgr);
        ...
        private void Setup()
        {
            ...
            //Basic Data
            Basic_Type1.ItemsSource = Enum.GetValues(typeof(Type));
            Basic_Type2.ItemsSource = Enum.GetValues(typeof(Type));
        ...
        private void Load(PokemonData _LoadData = null)
        {
            ...
            try
            {
                Basic_Type1.SelectedItem = LoadData.Species_Type1;
            }
            catch
            {
            }
            try
            {
                Basic_Type2.SelectedItem = LoadData.Species_Type2;
            }
            catch
            {
            }
            ...
            try
            {
                ...
                foreach (var EL in LoadData.Evolutions)
                {
                    ...
                    link.LinkData = EL;
                    link.PokemonName = Mgr.SaveData.PokedexData.Pokemon.Find(x => x.Species_DexID == EL.Pokemon_Evo)
                        .Species_Name;
        ...
        private void Save()
        {
            ...
            PokemonData.Species_Type1 = (Type) Basic_Type1.SelectedItem;
            PokemonData.Species_Type2 = (Type) Basic_Type2.SelectedItem;
        ...
        private bool ValidateSettings()
        {
            ...
            try
            {
                ...
                else if (Mgr.SaveData.PokedexData.Pokemon
                             .FindAll(x => x.Species_Name.ToLower() == Basic_Name.Text.ToLower()).Count >= 1 && !Update)
                {
                ...
                if (Basic_Type1.SelectedIndex == -1
                ) // Check if the Primary Type is set, if not set it will fail the validation check
                {
                    MessageBox.Show("You have not selected the pokemons primary type!",
                        "Basic Information Error -> Typeing Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    Pass = false;
                }
                else if (Basic_Type2.SelectedIndex == -1
                ) // Check if the Secondary Type is set, if not set it to Primary Type
                {
                    MessageBox.Show("You have not selected the pokemons secondary type, making pokemon Solo Type",
                        "Basic Information Notice -> Typeing Noteice", MessageBoxButton.OK,
        ...
        {
            var link = new Move_Link(); // Create a new MoveLink Window to create a link with
            var add = link
        ...
        private void EditLinkMove_Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            var link = new Move_Link((Link_Moves) Moves_List
                .SelectedItem); // Creates a new MoveLink Window to modify link with
        ...
        {
            var link = new Link_Evolution(); // Create a new EvoLink Window to create a link with
            var add = link
                .ShowDialog(); // Shows the Link Window, Creates a Dialog to return true if it added successfully

            if (add == true)
            {
                var name = Mgr.SaveData.PokedexData.Pokemon.Find(x => x.Species_DexID == link.LinkData.Pokemon_Evo)
                    .Species_Name;
        ...
        private void EditLinkEvo_Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            var link = new Link_Evolution(((EvoLinks) FormsAndEvos_List.SelectedItem)
